...
export default function OrderList() {
  ...
  const loadOrders = useCallback(async () => {
    try {
      const res = await fetch(`${apiUrl}/ordenes/`, { credentials: "include" });
      const data = await res.json();

      // agregamos la propiedad "categoria" derivada en el front
      const withCategoria = data.map((o) => ({
        ...o,
        categoria: deriveCategoria(o),
      }));

      const sortedOrders = withCategoria.sort((a, b) => a.id - b.id);
      setOrders(sortedOrders);
    } catch (error) {
      console.error(error);
    }
  ...
  const sortedOrders = useMemo(() => {
    ...
    return [...filteredOrders].sort((a, b) => {
      // soporta "Sucursal.nombre" y "categoria"
      const valueA =
        sortColumn === "Sucursal.nombre"
          ? a.Sucursal?.nombre ?? ""
          : a[sortColumn] ?? "";
      const valueB =
        sortColumn === "Sucursal.nombre"
  ...
  const handleSave = async () => {
    try {
      ...
      setEditingOrderId(null);
      setIsEditing(false);
      // recarga para recalcular categoría si cambió algo
      loadOrders();
    } catch (error) {
      console.error(error);
    }
  ...
  const indexOfFirstOrder = indexOfLastOrder - ordersPerPage;
  const currentOrders = [...sortedOrders]
    .reverse() // mantiene tu comportamiento original
    .slice(indexOfFirstOrder, indexOfLastOrder);
      ...
      <Table striped bordered hover>
        <thead>
          <tr>
            ...
            <th onClick={() => handleSort("peso_total")} style={{ cursor: "pointer" }}>Peso total</th>
            <th onClick={() => handleSort("Sucursal.nombre")} style={{ cursor: "pointer" }}>Sucursal</th>
            {/* NUEVA COLUMNA: Categoría (derivada de los productos) */}
            <th onClick={() => handleSort("categoria")} style={{ cursor: "pointer" }}>Categoría</th>
            {!(context.user && context.user.rol_id === 4) && <th>Operaciones</th>}
          </tr>
        </thead>
        <tbody>
