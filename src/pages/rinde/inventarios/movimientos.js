...
export default function MovimientosInternos() {
  ...
  const nextPage = () => {
    if (currentPage < Math.ceil(movimientosFiltradosPorTipo.length / movimientosPerPage)) {
  ...
  return (
    <Container>
      ...
      <div className="d-flex justify-content-center align-items-center">
        ...
        <span className="mx-2">
          Página {currentPage} de {Math.ceil(movimientosFiltradosPorTipo.length / movimientosPerPage)}
        </span>
        <Button
          onClick={nextPage}
          disabled={currentPage === Math.ceil(movimientosFiltradosPorTipo.length / movimientosPerPage) || loading}
        >
          <BsChevronRight />
        </Button>
      </div>
