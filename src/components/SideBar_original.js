...
import {
  ...
  FiChevronDown,
  FiChevronRight,
  FiCalendar, FiShield, FiMessageCircle, FiGift
} from "react-icons/fi";
    ...
    <Nav defaultActiveKey="/" className="flex-column sidebar sb-root">
      ...
      <div className="sb-body">
        ...
        {showReturnButton && (
          <>
            ...
            {/* ======= CONTENIDO AUDITORIA ATENCION ======= */}
            {showAuditoriaAtencionItems && (
              <>

                {can("doc:create") && (
                  <Link to="/promociones" className="nav-link" onClick={handleLinkClick}>
                    Promociones
                  </Link>
                )}

                {can("doc:create") && (
                  <Link to="/bot/product-meta" className="nav-link" onClick={handleLinkClick}>
                    Productos (IA)
                  </Link>
                )}
                {can("doc:create") && (
                  <Link to="/bot/conversations" className="nav-link" onClick={handleLinkClick}>
                    Conversaciones
                  </Link>
                )}

                {can("doc:create") && (
                  <Link to="/bot/branch-meta" className="nav-link" onClick={handleLinkClick}>
                    Sucursales (IA)
                ...
                {can("doc:create") && (
                  <Link to="/bot/benefit-meta" className="nav-link" onClick={handleLinkClick}>
                    Beneficios
                  </Link>
                )}

                {can("doc:create") && (
                  <Link to="/bot/event-meta" className="nav-link" onClick={handleLinkClick}>
                    Eventos
                  </Link>
                )}
                {/* Próximos módulos */}
      ...
      */}
            ...
            {showFidelizacionItems && (
              <>
                {can("fidelizacion:view") && (
                  <Link to="/fidelizacion/dashboard" className="nav-link" onClick={handleLinkClick}>
                    Dashboard
                  </Link>
                )}

                {can("fidelizacion:comercios") && (
                  <Link to="/fidelizacion/comercios" className="nav-link" onClick={handleLinkClick}>
                    Comercios Asociados
                  </Link>
                )}

                {can("fidelizacion:campanias") && (
                  <Link to="/fidelizacion/campanias" className="nav-link" onClick={handleLinkClick}>
                    Campañas
                  </Link>
                )}

                {can("fidelizacion:premios") && (
                  <Link to="/fidelizacion/premios-clientes" className="nav-link" onClick={handleLinkClick}>
                    Premios Clientes
                  </Link>
                )}
                ...
                {can("fidelizacion:puntos") && (
                  <Link to="/fidelizacion/puntos-comercio" className="nav-link" onClick={handleLinkClick}>
                    Puntos Comercio
                  </Link>
                )}

                {can("fidelizacion:premiosComercio") && (
                  <Link to="/fidelizacion/premios-comercios" className="nav-link" onClick={handleLinkClick}>
                    Premios Comercios
                  </Link>
                )}

                {can("fidelizacion:canjesComercio") && (
                  <Link to="/fidelizacion/canjes-comercios" className="nav-link" onClick={handleLinkClick}>
                    Canjes Comercios
                  </Link>
                )}

                {can("fidelizacion:fraude") && (
                  <Link to="/fidelizacion/alertas-fraude" className="nav-link" onClick={handleLinkClick}>
                    Alertas Fraude
                  </Link>
                )}
              </>
